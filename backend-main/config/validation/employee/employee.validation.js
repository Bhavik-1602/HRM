...
export const loginSchema = Joi.object({
    work_email: Joi.string().email().required().messages({
        ...
        'string.work_email': 'Invalid work_email format.'
    }),
...
export const updateEmployeeProfileSchema = Joi.object({
  ...
  personal_phone_number: Joi.string().pattern(/^\d{10}$/).optional().allow("").messages({
    "string.pattern.base": "Personal phone number must be exactly 10 digits.",
  }),

  dob: Joi.date().iso().optional().allow("").messages({
    "date.base": "Invalid date format. Please enter a valid date in YYYY-MM-DD format.",
  }),

  gender: Joi.string().valid("male", "female", "other", "not specified").optional().messages({
    "any.only": "Gender must be either 'male', 'female', or 'other'.",
  }),

  marital_status: Joi.string().valid("married", "single", "other", "not specified").optional().messages({
  ...
  department: Joi.string().trim().min(2).max(100).optional().allow("").messages({
  ...
  permanent_address: Joi.object({
    ...
    postal_code: Joi.string().pattern(/^\d{5,6}$/).allow(null, "").optional().messages({
  ...
  education_details: Joi.object({
    ...
    passing_year: Joi.string().trim().pattern(/^\d{4}$/).allow(null, "").optional().messages({
  ...
  original_doc_name: Joi.string().trim().max(255).allow(null, "").optional().messages({
  ...
  proof_doc_name: Joi.string().trim().max(255).allow(null, "").optional().messages({
