...
const forgotPassword = async (req, res) => {
  try {
    ...
    hr.reset_password_token = resetToken;
    hr.reset_password_expires = Date.now() + 3600000; // 1 hour expiry
    await hr.save();
...
const resetPassword = async (req, res) => {
  try {
    ...
    const isSamePassword = await bcrypt.compare(new_password, hr.password);
    if (isSamePassword) {
      return res
        .status(STATUS_CODES.BAD_REQUEST)
        .json(
    ...
    hr.reset_password_token = null;
    hr.reset_password_expires = null;
