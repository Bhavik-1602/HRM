...
const forgotPassword = async (req, res) => {
  try {
    ...
    admin.reset_password_expires = Date.now() + 3600000; // 1 hour expiry
    await admin.save();

    const resetURL = `${process.env.FRONTEND_URL}/admin/reset-password?token=${resetToken}&work_email=${work_email}`;
